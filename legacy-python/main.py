...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .note-save-badge.note-save-error { color: #ef4444; }
        ...
        <div class="notes-grid">
          <div class="note-card">
            <div class="note-card-header">
              ...
              <span class="note-save-badge note-save-error" :class="noteSaveError.kpi_summary ? 'visible' : ''">저장 실패 ✗</span>
            </div>
          ...
          <div class="note-card">
            <div class="note-card-header">
              ...
              <span class="note-save-badge note-save-error" :class="noteSaveError.project_progress ? 'visible' : ''">저장 실패 ✗</span>
            </div>
          ...
          <div class="note-card">
            <div class="note-card-header">
              ...
              <span class="note-save-badge note-save-error" :class="noteSaveError.next_week_strategy ? 'visible' : ''">저장 실패 ✗</span>
            </div>
...
function kpiApp() {
  return {
    ...
    noteSaveError: { kpi_summary: false, project_progress: false, next_week_strategy: false },
    weeklyTasks: [],
    weeklyTaskNew: '',
    ...
    _setQuillContent(quill, raw) {
      if (!raw) { quill.setContents([], 'silent'); return; }
      try {
        const delta = JSON.parse(raw);
        if (delta?.ops) { quill.setContents(delta, 'silent'); return; }
      } catch {}
      // 플레인 텍스트 fallback
      quill.setText(raw, 'silent');
    },

    async _saveNoteFromQuill(field) {
      if (!window._noteEditors?.[field]) return;
      // 저장 시점에 모든 에디터 내용을 최신으로 읽어 payload 완성
      this.NOTE_FIELDS.forEach(f => {
        if (window._noteEditors?.[f]) {
          this.notes[f] = JSON.stringify(window._noteEditors[f].getContents());
        }
      });
    ...
    async loadNotes() {
      try {
        ...
        this.noteSaveError = { kpi_summary: false, project_progress: false, next_week_strategy: false };
    ...
    async saveNote(field) {
      try {
        ...
        this.noteSaveError[field] = false;
        setTimeout(() => { this.noteSaved[field] = false; }, 2000);
      } catch(e) {
        console.error('[notes] 저장 실패:', e);
        this.noteSaveError[field] = true;
        setTimeout(() => { this.noteSaveError[field] = false; }, 4000);
      }
    },
